...
def test_expected_symbols_present() -> None:
    assert hasattr(overture_stac, "build_release_catalog")
...
def test_build_release_catalog_has_docstring() -> None:
    doc = overture_stac.build_release_catalog.__doc__
...
def test_schema_version_wrong_type_raises_typeerror() -> None:
    ...
    with pytest.raises(TypeError):
        overture_stac.build_release_catalog("2026-07-22.0", 123)  # type: ignore[arg-type,unused-coroutine]
...
async def test_bogus_data_uri_raises_overturestacerror() -> None:
    ...
    with pytest.raises(overture_stac.OvertureStacError):
        await overture_stac.build_release_catalog(
            "2026-07-22.0",
            "1.18.0",
...
async def test_build_release_catalog_returns_awaitable() -> None:
    # Sanity: calling build_release_catalog must produce something awaitable so callers
    # can `await` it. Cancel afterwards to avoid running the actual I/O.
    fut = overture_stac.build_release_catalog(
        "2026-07-22.0",
        "1.18.0",
        data_uri="s3://nope",
        extras_uri=None,
